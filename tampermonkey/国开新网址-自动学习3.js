    ...
    function start() {
        ...
        if (/\/course\/\d+\/ng/.test(pathname) && hash==="#/" && $('#AutoLearn').length==0) {
            ...
            W.Surplus = async function (obj) {
                ...
                if(array.length>0){
                    ...
                    for (let index = 0; index < array.length; index++) {
                        setTimeout(() => {
                            const percent = Math.round(100 * (index + 1) / array.length);
            ...
            W.AutoLearn = async function (obj) {
                ...
                const module_ids = [];
                $('.modules>.module-sort>.module').each(function (index, el) {
                    ...
                    success: function (res) {
                        ...
                        if (activities.length > 0) {
                            for (let index = 0; index < activities.length; index++) {
                                setTimeout(() => {
                                    const percent = Math.round(100 * (index + 1) / activities.length);
                                    $dom.innerHTML = `${percent}%`;
