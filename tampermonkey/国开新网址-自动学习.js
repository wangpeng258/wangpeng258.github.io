...
(function () {
    ...
    function start() {
        ...
        if (/\/course\/\d+\/ng/.test(pathname) && hash==="#/" && $('#AutoLearn').length==0) {
            ...
            <div id="AutoLearn" >
              <button onclick="Surplus(this)" type="button" class="button button-green small gtm-label" style="position: fixed;bottom: 50px;left: 50%;transform: translateX(-50%);z-index:999;">学习视频</button>
              ...
              <button onclick="AutoLearn(this)" type="button" class="button button-green small gtm-label" style="position: fixed;bottom: 150px;left: 50%;transform: translateX(-50%);z-index:999;">学习全部</button>
            </div>
            `);
            function toS(str){
            ...
            W.Surplus = async function (obj) {
                ...
                if(array.length>0){
                    ...
                    for (let index = 0; index < array.length; index++) {
                        setTimeout(() => {
                            document.title = `${index+1}/${array.length}`
                    ...
                    success: function (res) {
                        ...
                        if (activities.length > 0) {
                            for (let index = 0; index < activities.length; index++) {
                                setTimeout(() => {
                                    document.title = `${index+1}/${activities.length}`
