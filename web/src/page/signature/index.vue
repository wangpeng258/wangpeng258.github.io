...
<script>
import { isObjEmpty, dateFormat,isPhone } from '@/libs/utils';
...
export default {
    ...
    methods: {
        ...
        guiInit() {
            this.gui && this.gui.destroy();
            this.gui = new GUI();
            const optionsFolder = this.gui.addFolder(`参数`);
            optionsFolder.open();
            optionsFolder.addColor(this.controls, 'penColor').name('画笔颜色').onChange((value) => {
                this.signaturePad && (this.signaturePad.penColor = value)
            });
            optionsFolder.add(this.controls, 'minWidth', 0.1, 10).name('最小宽度').step(0.1).onChange((value) => {
                this.signaturePad && (this.signaturePad.minWidth = value)
            });
            optionsFolder.add(this.controls, 'maxWidth', 0.1, 10).name('最大宽度').step(0.1).onChange((value) => {
                this.signaturePad && (this.signaturePad.maxWidth = value)
            });
            optionsFolder.add(this.controls, 'throttle', 1, 50).name('风门').step(1).onChange((value) => {
                this.signaturePad && (this.signaturePad.throttle = value)
            });

            const funFolder = this.gui.addFolder(`方法`);
            funFolder.close();
            this.controls.undo = () => {
            ...
            funFolder.add(this.controls, 'undo').name(`撤消`);

            this.controls.clear = () => {
                this.signaturePad && (this.signaturePad.clear());
                this.gui.close();
            }
            funFolder.add(this.controls, 'clear').name(`清除`);


            this.controls.savePng = () => {
                if (this.signaturePad.isEmpty()) {
                    this.$toast("请先提供签名");
                } else {
                    var dataURL = this.signaturePad.toDataURL(`image/png`, 1);
                    this.download(dataURL, `signature_${dateFormat('YYYYmmddHHMMSS')}.png`);
                    this.gui.close();
                }
            }
            funFolder.add(this.controls, 'savePng').name(`下载PNG`);

            this.controls.saveJpg = () => {
                if (this.signaturePad.isEmpty()) {
                    this.$toast("请先提供签名");
                } else {
                    var dataURL = this.signaturePad.toDataURL(`image/jpeg`, 1);
                    this.download(dataURL, `signature_${dateFormat('YYYYmmddHHMMSS')}.jpg`);
                    this.gui.close();
                }
            }
            funFolder.add(this.controls, 'saveJpg').name(`下载JPG`, 1);

            this.controls.saveSvg = () => {
                if (this.signaturePad.isEmpty()) {
                    this.$toast("请先提供签名");
                } else {
                    var dataURL = this.signaturePad.toDataURL(`image/svg+xml`);
                    this.download(dataURL, `signature_${dateFormat('YYYYmmddHHMMSS')}.svg`);
                    this.gui.close();
                }
            }
            funFolder.add(this.controls, 'saveSvg').name(`下载SVG`);
            isPhone() && this.gui.close();
            // this.gui.open();
            // this.gui.close();
        },
        createSignature() {
            const canvas = this.$refs.canvas;
            this.signaturePad = new SignaturePad(canvas, this.controls);
        },
        resizeCanvas() {
            const canvas = this.$refs.canvas;
